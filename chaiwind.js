  ...
  function parseLayout(className) {
    ...
    for (const [prefix, handler] of LAYOUT.DYNAMIC) {
      if (className.startsWith(prefix)) {
        ...
        // use toRem for spacing-based dynamic classes, raw value for grid/z/order
        const needsRem = ["chai-gap-x-","chai-gap-y-","chai-gap-","chai-w-","chai-h-","chai-min-w-","chai-min-h-","chai-max-w-","chai-max-h-","chai-top-","chai-bottom-","chai-left-","chai-right-"].includes(prefix);
        const val = needsRem ? toRem(suffix) : suffix;
        if (!val) return {};
